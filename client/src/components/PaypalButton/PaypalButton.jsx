...
function PaypalButton({input} ) {
		...
		if (currentUser.id) {
			try {
				dispatch(
					editProductStock({
						...userOrderProducts,
						stock: userOrderProducts.stock.map((stock) => (stock == null ? null : stock - 1)),
					})
				);
			...
			try {
				...
				dispatch(
					editProductStock({
						...
						stock: guestProducts.map((product) => (product.stock == null ? null : product.stock - 1)),
					})
				);
				...
				Swal.fire({
					...
					.then(() => {
						return (guestOrder = JSON.parse(localStorage.getItem('guestOrderDetails')));
					})
					.then(() => {
						dispatch(
							sendEmailOrderSuccess({
	...
	const handlePayError = (error) => {
		Swal.fire({
			title: 'Error',
			text: `We could not complete the purchase please try again`,
			icon: 'warning',
	...
	const handleCancelPay = (data) => {
		...
		Swal.fire({
			title: 'Canceled',
			text: `the purchase was canceled, please take the transaction number: ${data.id}`,
			icon: 'warning',
			confirmButtonText: 'OK',
		});
